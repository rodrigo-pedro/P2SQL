...
#     'gpt-4-1106-preview': [0.9833333333333333, 0.95, 0.9333333333333333, 0.8]
# }

def load_accuracy_from_json(file_path):
    with open(file_path, 'r') as f:
        data = json.load(f)
        return data.get('accuracy', None)

scenarios = ['only_results', 'question_results', 'question_results_thought', 'rebuff']
...
for model in models:
    ...
    for scenario in scenarios:
        file_path = os.path.join(base_path, f'{scenario}_stats_{model}.json')
        accuracy = load_accuracy_from_json(file_path)
        if accuracy is not None:
            accuracies.append(accuracy)
    detections[model] = accuracies

assert len(detections[models[0]]) == len(detections[models[1]]) == len(versions)
...
file_path = os.path.join(base_path, "llm_guard_detections_with_rebuff.pdf")
