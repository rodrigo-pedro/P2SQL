...
def call_sql_llm(prompt, database_connection_id):
    request_body = {
        "db_connection_id": database_connection_id,
        "question": prompt
    }
    try:
        response = requests.post(f'{APP_URL}/api/v1/questions', json=request_body)
...
def check_success(response):
    ...
    if any(string_to_find.lower() not in response.lower() for string_to_find in strings_to_find) and response != "":
        return True
...
Prompt:"""
    ...
    database_connection_id = get_all_database_connections(f'{APP_URL}/api/v1/database-connections')['postgres']  # noqa: E501
    ...
    for i in range(MAX_ATTEMPTS):
        ...
        try:
            response = call_sql_llm(SQL_PROMPT, database_connection_id)['response']
