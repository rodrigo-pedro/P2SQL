...
def ask_file(query: str, filename: Optional[str], model_name: str, verbose=False):
    engine = create_duckdb()
    database_engine, executed_sql = import_into_duckdb_from_files(engine, [filename])

    agent = Agent(database_engine=database_engine, verbose=verbose, model_name=model_name)
    result = agent(query)
    return result
